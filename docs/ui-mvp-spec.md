...
- copyable CLI commands for `worker codex run-list`, `run-show`, `run-status`, `preflight`, `run-plan`, `run-plan-list`, `run-plan-show`, `report-template`, `report-validate`, `report-import`, `report-show`, `report-list`, plus post-review `queue-next` and `queue-complete-item` guidance
...
This page is read-only. It does not run Codex, import/upload reports from the UI, approve execution, execute target commands, run validation/build/test, commit, push, restore backups, modify schedulers, edit target files, call model APIs, or complete queue/task items.
