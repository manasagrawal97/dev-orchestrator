...
- copyable CLI commands for `worker codex prepare-next`, `queue-status`, `queue-status --item`, `flow-summary`, `run-list`, `run-show`, `run-status`, `preflight`, `run-plan`, `run-plan-list`, `run-plan-show`, `execute-preview`, guarded `execute --confirm-execute`, `execute-log`, `report-template`, `report-validate`, `report-import`, `report-show`, `report-list`, `review-template`, `review-attach-evidence`, `review-record`, `review-show`, `review-list`, plus post-review `queue-next` and explicit `queue-complete-item` guidance
- read-only queue-worker linkage fields: selected item source, source handoff, linked worker run, linked run plan, current item review status, validation evidence status, completion readiness, completion blockers, latest execution status, latest report status, and next CLI command
...
Delivery visibility is documented in `docs/delivery-safety-design.md`. TASK-DEVO-105 adds read-only delivery readiness artifacts and API endpoints, TASK-DEVO-106 adds delivery plan/approval artifacts and endpoints, TASK-DEVO-107 adds delivery report metadata, TASK-DEVO-108 adds guarded CLI commit result metadata, and TASK-DEVO-109 adds guarded CLI push result metadata. TASK-DEVO-113 adds recovery metadata for retryable guarded commit failures and copyable `delivery report-refresh` guidance. TASK-DEVO-114 adds CLI-only `delivery commit-diagnostics` for index-lock and permission failure investigation. TASK-DEVO-115 documents that live delivery commit/push should run from normal local PowerShell with `.\.venv\Scripts\devo.exe`, not from restricted UI/Codex contexts unless diagnostics prove index-lock creation works. TASK-DEVO-116 adds guarded-commit index-lock preflight before staging so restricted contexts fail with retryable metadata instead of partially staging files. TASK-DEVO-116A separates docs/README secret-safety warnings from real secret blockers. TASK-DEVO-117 adds a read-only latest delivery summary that distinguishes empty clean checks from meaningful delivery work. TASK-DEVO-118 adds trusted local runner artifacts and a copyable normal-PowerShell runner command, but still no UI commit/push action. TASK-DEVO-119 keeps Phase 1 closure focused on workflow reliability before Phase 2 intelligence. A future Delivery page may show readiness, changed-file scope, validation/review evidence, plan status, approval status, blockers, warnings, commit/push result metadata, recovery status, latest useful delivery state, runner request status, diagnostics command guidance, and copyable CLI commands. UI commit/push buttons remain deferred.
