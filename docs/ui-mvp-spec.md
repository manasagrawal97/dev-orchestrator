...
Shows Codex worker run and imported report visibility for assisted manual handoff.

Sections:

- worker run count, latest worker run id/status, latest report status, and latest next action
- worker run list with run id, status, mode, source handoff/queue/item/task, report status, timestamps, and next action
- selected worker run detail with worker type, mode, prompt/transcript/report paths, target repo path, allowed/forbidden scope, validation expectations, safety boundaries, status note, and next action
...
- copyable CLI commands for `worker codex run-list`, `run-show`, `run-status`, `report-template`, `report-validate`, `report-import`, `report-show`, `report-list`, plus post-review `queue-next` and `queue-complete-item` guidance

Primary user question: "What did the manual Codex worker run report, what evidence exists, and what should I review before touching queue/task state?"

This page is read-only. It does not run Codex, import/upload reports from the UI, execute target commands, run validation/build/test, commit, push, restore backups, modify schedulers, edit target files, call model APIs, or complete queue/task items.
