...
TASK-DEVO-099 found that the detected WindowsApps `codex.exe` path can pass preflight/preview but fail at `CreateProcess` with access denied. The next usability work should harden launcher-path selection and launch-exception recovery before retrying real supervised execution or adding delivery automation.
