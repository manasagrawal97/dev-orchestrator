...
TASK-DEVO-133 and TASK-DEVO-134 reduce that friction with `queue-worker-step` and `queue-worker-loop`. The step command performs one safe queue-worker transition; the loop command repeats those one-task-at-a-time transitions until it reaches missing worker/review/validation evidence, pending trusted delivery, policy drift, terminal state, no eligible item, or max steps. TASK-DEVO-191 later adds explicit optional validation execution at the validation boundary. This remains supervised operation: Devo does not run real Codex from this loop, bypass review, run trusted delivery, commit, push, or parallelize tasks.
...
TASK-DEVO-181 shifts the next usability slice back to Manas's planning burden. `devo project intake-plan --project <project> --from-file <goal.md> --confirm-create` parses rough Markdown headings into a workspace-only intake bundle with candidate tasks, suggested batch/queue/policy drafts, allowed files, risk notes, and next commands. It is deterministic, preview-first, and does not create approvals, real queues, Codex runs, validation, delivery, commits, pushes, or target edits. TASK-DEVO-182 adds `devo project intake-materialize --project <project> --intake <INTAKE-ID> --confirm-materialize`, which turns a reviewed intake into real draft backlog tasks, a draft batch, a draft queue, and a draft execution policy while preserving scope, validation, delivery, and risk notes. TASK-DEVO-183 proves those materialized artifacts can start a real gated DevOrchestrator slice, and TASK-DEVO-184 adds `devo project intake-next-slice --project <project> --intake <INTAKE-ID>` so Devo can recommend the smallest low-risk slice and exact narrow-policy commands before approval. TASK-DEVO-186 improves that recommendation by deprioritizing already-performed intake setup/review tasks once intake and materialization artifacts exist, and by preserving validation notes in generated narrow policy commands. TASK-DEVO-187 adds `devo project intake-policy-create-next --project <project> --intake <INTAKE-ID> --confirm-create-policy`, which converts the recommended next slice into one narrow draft execution policy without requesting approval, approving, running workers, creating delivery, committing, or pushing. The materialized artifacts and generated draft policy still require explicit approval before any worker or delivery flow starts.
...
The model-migration handoff records the product priority in `docs/ai-handoff.md`: supervised Codex CLI automation comes before UI/dashboard polish. TASK-DEVO-191 adds optional `queue-worker-loop --auto-validation` behavior: the default remains manual, explicit confirmed mode runs approved policy validation only after worker completion and passed review, and failed or blocked validation stops before delivery. The loop may create the existing delivery request after validation passes, but it never runs the trusted runner, stages, commits, or pushes. TASK-DEVO-193 adds optional `queue-worker-loop --auto-worker`: confirmed mode reuses the existing one-item worker preparation/subprocess/strict-JSON-ingest path for the selected run and stops at review; dry-run does not launch the subprocess. TASK-DEVO-194 adds bounded bulk approval for already-existing eligible requested policies. TASK-DEVO-195 adds the supervised `auto-run-approved` sequential loop over one already-approved bundle while retaining each existing gate.
...
TASK-DEVO-193 composes only the worker boundary. `queue-worker-loop --auto-worker --confirm-loop` runs and ingests at most one configured Codex subprocess for the exact selected queue-worker run, then stops at review. Missing output, invalid JSON, blocked/failed status, usage limits, or run-selection ambiguity stop safely. It does not auto-review, auto-validate, create delivery, invoke trusted delivery, stage, commit, push, or run workers in parallel.

TASK-DEVO-194 implements one narrow execution-policy approval-bundle workflow. Request preview is read-only; confirmed request accepts two through ten distinct existing requested policies and requires approved batch references, explicit queues/tasks/queue items, allowed and forbidden file patterns, validation commands, positive limits, and retained review/validation gates. Only low-risk child policies whose referenced queue items are still `pending` are eligible. The artifact pins child scope fingerprints and aggregate bounds. Approval is separately previewed and confirmed, rechecks every child for status/reference/scope drift and queue-item actionability, then records normal approval state on each policy. The bundle does not create policies or queue work and does not run workers, review, validation, delivery, trusted runner, commit, or push.

TASK-DEVO-195 implements `devo project auto-run-approved --project <project> --bundle <PAB-ID>`. V1 accepts only an already-approved TASK-DEVO-194 bundle. Every invocation rechecks bundle status, all child approvals, pinned scope fingerprints and task/queue-item references, low risk, explicit file and validation scope, positive bounds, `max_tasks_per_run=1`, required review/validation gates, auto-delivery/auto-push permissions, approved batch/queue linkage, non-overlapping child ownership, actionable queue state, and at most one active bundle run before any mutation. Dry-run writes nothing, does not depend on scheduler health, and previews the selected policy and queue item, resume/create decision, and ordered gate sequence. Confirmed mode retains the healthy-scheduler gate by default, then advances only the active child or first pending child through the existing one-worker strict-JSON service, conservative deterministic reviewer, approved validation runner, delivery-request creation, and completed trusted-runner reconciliation. It stops on any blocker or pending trusted delivery and never invokes the trusted runner, stages, commits, pushes, or executes workers in parallel. Repeated multi-child live dogfood remains the next evidence step.
