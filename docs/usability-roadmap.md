...
The first real supervised Codex launch should follow `docs/runbooks/real-codex-supervised-dry-run.md`. That runbook keeps the first real run no-op/docs-only, targets DevOrchestrator first, and treats success as evidence that the approval/preview/execution/report/review gates worked rather than as implementation productivity.

TASK-DEVO-099 found that the detected WindowsApps `codex.exe` path can pass preflight/preview but fail at `CreateProcess` with access denied. TASK-DEVO-100 blocked that path and catches launch failures; TASK-DEVO-101 found no safe real launcher on this machine; TASK-DEVO-102 adds the wrapper strategy. The next real retry should only proceed after `devo worker codex doctor` reports a safe real executable or wrapper launcher.
...
5. Retry supervised Codex CLI launch only after doctor reports a safe real executable or wrapper launcher.
