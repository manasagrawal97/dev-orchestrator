...
- TASK-DEVO-151: One-task Codex subprocess execution v1 - completed
- TASK-DEVO-152: Real Codex subprocess dogfood preparation on disposable `Dogfood152` reached `codex-worker-run-preview`; real launch is deferred to normal PowerShell using the prepared command in `docs/dogfood/task-devo-152-real-codex-subprocess-dogfood.md`
- TASK-DEVO-153: Codex subprocess dogfood hardening - completed; default args now use `codex exec -s workspace-write --output-last-message`, prompts are passed on stdin, and worker ingest gives stricter JSON guidance.
- TASK-DEVO-154: Batch Codex-worker loop design - completed in `docs/architecture/codex-worker-batch-loop-design.md`
- TASK-DEVO-155: Batch Codex-worker loop v1 - completed; `devo project codex-worker-batch-run` coordinates one approved queue item through prepare, one configured subprocess, strict JSON ingest, and review-gate stop with fake-worker tests
- TASK-DEVO-156: Batch-run fake-worker dogfood - completed in `docs/dogfood/task-devo-156-batch-run-fake-worker-dogfood.md`; one disposable queue item reached strict JSON ingest, stopped at review, advanced through manual review/validation evidence, and created a trusted delivery request
- TASK-DEVO-157: Batch-run dogfood polish and disposable delivery readiness - completed; disposable repo upstream guidance, no-upstream delivery warnings, and `execution-policy-create` changed-file-limit output are clearer
- TASK-DEVO-158: Real Codex batch-run one-item dogfood - completed in `docs/dogfood/task-devo-158-real-codex-batch-run-one-item.md`; disposable `Dogfood158` reached real Codex subprocess execution, JSON ingest, review gate, manual review/validation, trusted runner delivery, push, and clean final repo state
- TASK-DEVO-159: Real Codex batch-run polish - completed; usage-limit false positives from echoed schema text are reduced, validation evidence output labels shared evidence artifacts clearly, and completed trusted-delivery next actions are terminal
- TASK-DEVO-160: Multi-item fake-worker batch continuation dogfood - completed in `docs/dogfood/task-devo-160-multi-item-fake-worker-batch-continuation.md`; disposable `Dogfood160` completed three fake-worker queue items through manual evidence and trusted delivery
- TASK-DEVO-161: Batch continuation friction polish - completed; stale active retry runs tied to completed items no longer block next-item selection, retry runs link worker runs when possible, completed queues get terminal guidance, and push recovery points at `runner-recover-push`
- TASK-DEVO-162: Real Codex multi-item batch dogfood - completed in `docs/dogfood/task-devo-162-real-codex-multi-item-batch-dogfood.md`; disposable `Dogfood162` completed two real Codex subprocess queue items one at a time through strict JSON ingest, manual review/validation, trusted runner delivery, and final all-completed guidance
- TASK-DEVO-163: Real Codex batch-run readiness checkpoint - completed in `docs/architecture/real-codex-batch-run-readiness-checkpoint.md`; documents safe disposable use, narrow DevOrchestrator use, PersonalOS deferral, manual gates, and trusted-runner-only delivery
...
- Next: resume `QWR-0010` through explicit whitespace-tolerant check/apply, then complete post-apply evidence and trusted-delivery continuation before using patch apply on broader live source-code work
...
TASK-DEVO-145 adds the design-only Codex worker launch/integration plan in `docs/architecture/codex-worker-launch-integration-design.md`. TASK-DEVO-146 implements prompt-file assisted worker preparation for one `waiting_worker` queue-worker run without running Codex, calling AI APIs, recording evidence automatically, validating, committing, pushing, or modifying target repositories. TASK-DEVO-147 adds JSON worker result ingest that preserves the raw result file and records queue-worker worker evidence schema v1 without running Codex, review, validation, delivery, commit, or push. TASK-DEVO-148 dogfoods that prompt-file/manual loop on disposable project `Dogfood148`, proving the prepare -> filled JSON result -> dry-run ingest -> confirmed ingest -> review evidence -> validation evidence -> trusted runner delivery path. TASK-DEVO-149 records the subprocess execution checkpoint and concludes Devo is ready only for a very narrow one-task subprocess v1. TASK-DEVO-150 adds workspace-only subprocess config and `codex-worker-run-preview`, which validates the planned command and preflight state without launching Codex, calling AI APIs, ingesting, reviewing, validating, delivering, committing, or pushing. TASK-DEVO-151 adds one-task subprocess execution with fake-command tests, artifact capture, and result classification. TASK-DEVO-152 prepared the real one-task dogfood on disposable `Dogfood152` through queue/policy/preparation/config/preview, but did not launch real Codex from inside Codex/sandbox. TASK-DEVO-153 hardens the default real Codex CLI command shape, stdin prompt passing, strict JSON ingest guidance, and completed-run next-action wording before another normal-PowerShell retry. TASK-DEVO-154 designs a future batch loop that still processes one approved queue item at a time, stops at evidence/safety gates, and keeps delivery trusted-runner-only. TASK-DEVO-155 implements that first narrow coordinator as `codex-worker-batch-run`, capped to one item/cycle and stopping after JSON ingest at the review gate. TASK-DEVO-156 dogfoods that coordinator against disposable `Dogfood156` with a fake subprocess worker and records the remaining delivery-readiness polish in `docs/dogfood/task-devo-156-batch-run-fake-worker-dogfood.md`. TASK-DEVO-157 completes that polish with clearer no-upstream delivery risk wording, execution-policy changed-file-limit hints, and disposable local bare remote setup guidance. TASK-DEVO-158 proves one real disposable Codex batch-run item end to end from normal PowerShell through trusted runner delivery, TASK-DEVO-159 polishes the readouts, TASK-DEVO-160 proves fake-worker continuation across three disposable queue items, TASK-DEVO-161 polishes the retry/completed-queue friction, TASK-DEVO-162 proves real Codex continuation across two disposable queue items, TASK-DEVO-163 records the readiness checkpoint before real-project use, TASK-DEVO-164 proves narrow live DevOrchestrator docs-only use, TASK-DEVO-165 adds a consolidated read-only batch-position summary, TASK-DEVO-166 dogfoods that summary on completed live policy `POL-0002`, TASK-DEVO-169 adds patch-proposal fallback evidence for blocked writes, TASK-DEVO-170 dogfoods that fallback, TASK-DEVO-171 designs reviewed patch apply, TASK-DEVO-172 adds show/check, TASK-DEVO-173 dogfoods show/check, and TASK-DEVO-174 adds explicit reviewed apply without marking review, validation, delivery, or queue completion.
