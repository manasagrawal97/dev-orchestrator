...
- TASK-DEVO-163: Real Codex batch-run readiness checkpoint - in progress in `docs/architecture/real-codex-batch-run-readiness-checkpoint.md`; documents safe disposable use, narrow DevOrchestrator use, PersonalOS deferral, manual gates, and trusted-runner-only delivery
- Next: polish real multi-item continuation ergonomics and output capture while preserving the one-item-at-a-time safety gates
