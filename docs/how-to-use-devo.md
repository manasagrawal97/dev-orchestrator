...
When Codex/sandbox has prepared and validated a change but cannot create `.git/index.lock`, use the trusted local runner flow. Codex runs `devo delivery runner-request --project <name> --message "<commit message>" --note "<note>"` to write a workspace-only request and changed-file snapshot. Manas then runs the printed normal PowerShell command, usually with `.\.venv\Scripts\devo.exe delivery runner-run --project <name> --request <requestId> --approver "Manas" --confirm-runner-delivery`. The runner still re-runs delivery check, verifies the changed-file snapshot, creates/approves delivery artifacts, prepares the report, runs commit-preview, guarded commit, push-preview, and guarded push. It is not background automation and does not bypass delivery gates.
