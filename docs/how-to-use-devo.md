...
Patch proposal application is intentionally separate from patch proposal capture. TASK-DEVO-171 designs future reviewed patch apply in `docs/architecture/reviewed-patch-apply-design.md`, and TASK-DEVO-172 adds the first safe inspection slice: `patch-proposal-show` plus explicit `patch-proposal-check --confirm-check`. The check command writes a workspace-only artifact and may run `git apply --check`, but it does not apply the patch, mutate queue state, record review, record validation, create delivery, commit, or push. Until a future apply workflow exists and is separately approved, do not apply patch proposals through Devo. Even after a future apply command exists, patch apply will not mean the queue item is complete: the operator must review the actual diff, run validation, record normal evidence, and deliver through the trusted runner.
