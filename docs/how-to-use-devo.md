...
Patch proposal application is intentionally separate from patch proposal capture. TASK-DEVO-171 designs future `patch-proposal-show`, `patch-proposal-check`, and `patch-proposal-apply` commands in `docs/architecture/reviewed-patch-apply-design.md`. Until that workflow exists and is approved, do not apply patch proposals through Devo. Even after a future apply command exists, patch apply will not mean the queue item is complete: the operator must review the actual diff, run validation, record normal evidence, and deliver through the trusted runner.
