...
The durable priority is supervised Codex CLI automation before UI/dashboard polish. TASK-DEVO-191 adds optional `queue-worker-loop --auto-validation` after TASK-DEVO-190's standalone validation runner. Continue from the next approved/pending task unless Manas redirects.
...
After materialization, run `intake-next-slice` before approval. It inspects the intake receipt plus the linked batch, queue, policy, and backlog tasks, then recommends the smallest low-risk next slice. If the materialized policy is broader than that slice, leave it as a draft and create a narrower execution policy from the materialized batch and queue. For example, use one task id from the materialized queue with exact allowed files, then request and approve only that smaller policy before running any queue-worker command. When validation notes exist, the generated narrow policy command includes one `--validation-command` flag per note so validation expectations do not disappear during slicing.
...
The queue-worker loop can call the same service explicitly at its validation gate:

```powershell
devo project queue-worker-loop --project MyProject --policy POL-0001 --run QWR-0001 --auto-validation --confirm-loop
```

Without `--auto-validation`, existing behavior is unchanged and the loop stops for manual validation evidence. With the flag, confirmed mode runs only validation commands from the approved policy after completed worker evidence and passed review. Passing validation lets the existing loop continue toward delivery-request creation; failed or blocked validation stops the loop without delivery. `--dry-run --auto-validation` never executes validation commands. The loop does not run the trusted runner, stage, commit, or push.

The same loop can optionally run and ingest one configured Codex worker at the worker gate:

```powershell
devo project queue-worker-loop --project MyProject --policy POL-0001 --run QWR-0001 --auto-worker --confirm-loop
devo project queue-worker-loop --project MyProject --policy POL-0001 --run QWR-0001 --auto-worker --dry-run
```

Without `--auto-worker`, worker handling remains manual. With the flag, confirmed mode binds execution to that queue-worker run, reuses the existing one-item Codex batch preparation/subprocess/strict-JSON-ingest path, and stops at review. Dry-run does not launch a subprocess or write worker artifacts. A blocked process, missing result, invalid JSON, usage limit, or non-completed worker result stops before review, validation, or delivery. This option does not auto-review, auto-validate, create delivery, invoke the trusted runner, stage, commit, push, or start parallel workers.
...
# Recheck and preview exactly one child without launching a worker or writing artifacts.
devo project auto-run-approved --project MyProject --bundle PAB-0001 --dry-run

# After human review of the bundle and preview, advance one child through the existing gates.
devo project auto-run-approved --project MyProject --bundle PAB-0001 --confirm-auto-run

# Preview continuous sequential supervision without writing artifacts or polling.
devo project auto-run-approved --project MyProject --bundle PAB-0001 --supervise --dry-run

# Supervise all eligible children sequentially, waiting only for external trusted-runner evidence.
devo project auto-run-approved --project MyProject --bundle PAB-0001 --supervise --poll-interval-seconds 5 --max-wait-seconds 600 --confirm-auto-run
```
...
`auto-run-approved` consumes one already-approved bundle; it never creates backlog, batch, queue, policy, or approval artifacts. Without `--supervise`, TASK-DEVO-195 behavior is unchanged: one invocation advances at most one child and stops while trusted delivery is pending. Dry-run is an independently usable read-only preview: it does not query trusted-runner scheduler health, and it shows the selected child policy, selected queue item, whether execution would resume a run or create one, and the ordered gates confirmed execution would use. Before execution it rechecks bundle approval, pinned fingerprints and references, low-risk status, positive bounds, one-task-per-run limits, non-overlapping child scopes, required review/validation gates, auto-delivery/auto-push permission, approved batch linkage, queue linkage, queue-item state, and the one-active-run invariant. Any drift blocks before a worker starts. Confirmed mode requires a healthy scheduler by default unless the operator explicitly supplies `--no-require-scheduler-healthy`; it selects only the existing active child or the first pending child in bundle order, then composes the existing one-worker strict-JSON path, objective deterministic review, approved validation commands, delivery-request creation, and completed trusted-runner reconciliation.

`auto-run-approved --supervise` is the explicit continuous mode. It repeats that same one-child path sequentially, polls only the existing delivery request/runner evidence, reconciles a child only after canonical trusted evidence proves a pushed commit, rechecks the approved bundle before selecting another child, and writes one durable supervisor artifact under `workspace/projects/<project>/planning/execution-policies/approval-bundles/supervisor-runs/<ABSR-ID>/`. Polling is bounded by positive `--poll-interval-seconds` and `--max-wait-seconds` values. `trusted_delivery_timeout` is a safe pause: the request is left pending and a later supervised invocation resumes it without creating a duplicate request or retrying the worker. Other terminal reasons include `bundle_completed`, `waiting_human_review`, `worker_blocked`, `worker_failed`, `usage_limit`, `validation_failed`, `trusted_delivery_failed`, `contradictory_durable_state`, and `policy_drift`. Supervised dry-run creates no artifacts, subprocesses, evidence, requests, or real polling sleeps. Neither mode invokes runner-watch or the trusted runner, stages, commits, pushes, retries failures automatically, or runs children in parallel. Deterministic review still does not prove semantic, architectural, business-logic, or code-quality correctness.

Confirmed supervision is restart-safe. It holds an operating-system lock scoped to the project and bundle, so a second same-bundle process stops before mutation; the lock is released automatically when the process exits. The supervisor atomically checkpoints its event sequence and cursor before and after each child advance, delivery wait, and reconciliation. After a crash or machine restart, rerun the same confirmed command. Devo reuses the one incomplete `ABSR-*` artifact, reconstructs completion from canonical queue and delivery evidence, resumes the existing queue-worker run or delivery request, and records a `supervisor_reentry` event. A completed or deliberately paused invocation remains terminal and a later invocation gets a new supervisor id.
...
devo project queue-worker-loop --project MyProject --policy POL-0001 --run QWR-0001 --auto-worker --confirm-loop
devo project queue-worker-loop --project MyProject --policy POL-0001 --run QWR-0001 --auto-review --confirm-loop
devo project queue-worker-loop --project MyProject --policy POL-0001 --run QWR-0001 --auto-validation --confirm-loop
...
devo project patch-proposal-show --project MyProject --run QWR-0001
devo project patch-proposal-check --project MyProject --run QWR-0001 --confirm-check
devo project patch-proposal-apply --project MyProject --run QWR-0001 --reviewed-by "Manas" --confirm-apply-patch
...
Queue-worker run artifacts live under `workspace/projects/<project>/planning/queue-worker-runs/`. The preferred assisted primitives are `queue-worker-step`, `queue-worker-loop`, `approved-queue-run`, `auto-run-approved`, and the fake-tested `codex-worker-batch-run`. `queue-worker-step` performs exactly one safe state transition: create one run for an approved policy, wait for worker evidence, advance review/validation gates, create a trusted runner request, or observe completed trusted delivery. `queue-worker-loop` repeats that one-step behavior until it reaches a safe stop condition: missing worker report, missing review, missing validation, pending trusted delivery, paused/failed/cancelled/blocked state, no eligible item, or max steps. Its default remains manual; explicit `--auto-worker` reuses the one-item Codex batch service, explicit `--auto-review` runs only the conservative low-risk deterministic reviewer, and explicit `--auto-validation` reuses the approved validation runner only after review passes. All three options are dry-run safe and stop on blockers. `approved-queue-run` remains the single-policy scheduler-aware wrapper. `auto-run-approved` is the bundle-bounded composition described above and still advances only one child at a time. `codex-worker-batch-run` is intentionally one item/cycle: it prepares the prompt package, runs one configured subprocess, ingests strict JSON, writes a batch-run artifact, and stops at review. `codex-worker-batch-summary` remains the read-only joined status view. No loop path bypasses approval or safety gates, invokes trusted delivery, stages, commits, pushes, or parallelizes work. `--continue-next` can start one next eligible item after a specified run completes safely, then stops at the next normal boundary. Manual worker/review/validation evidence commands remain available for inspection and recovery.

A retry does not generally allow a dirty repository. TASK-DEVO-196 adds one bounded exception for WIP left by the immediately linked parent queue-worker attempt. The parent subprocess artifact must contain a dirty-state fingerprint, and the retry rechecks the same branch and HEAD, exact staged/unstaged/untracked file sets, exact tracked and untracked content, approved policy/task file scope, forbidden patterns, and changed-file limit. The CLI and run artifact show `Inherited WIP verified` and the parent run id when that proof succeeds. If any baseline is missing or any content/file/lineage drift occurred, execution blocks before the worker subprocess starts; clean the unrelated state or resolve it manually instead of bypassing the check.

TASK-DEVO-158 proves the first one-item real Codex `codex-worker-batch-run` on disposable `Dogfood158`; see `docs/dogfood/task-devo-158-real-codex-batch-run-one-item.md`. The safe pattern is still: dry-run from Codex if useful, run the real Codex subprocess only from normal PowerShell against a disposable repo, review and validate manually, then deliver with the trusted runner. Real Codex batch-run is a narrow-policy, one-item-at-a-time operating mode with manual review, manual validation, and trusted-runner-only delivery. TASK-DEVO-159 polishes the readouts from that dogfood: schema/prompt mentions of `usage_limit` no longer trigger successful-run warnings, validation evidence labels shared queue-worker evidence artifacts clearly, and completed trusted-delivery runs end with no action needed. TASK-DEVO-160 then proves fake-worker continuation across three disposable items in `docs/dogfood/task-devo-160-multi-item-fake-worker-batch-continuation.md`. TASK-DEVO-161 makes that continuation smoother: stale queue-worker runs for completed items are ignored with warnings, retry runs link fresh worker runs when possible, completed queues get terminal guidance, and fake/scripted workers are told to parse the explicit `Task id:` prompt line. TASK-DEVO-162 proves real Codex continuation across two disposable items in `docs/dogfood/task-devo-162-real-codex-multi-item-batch-dogfood.md`. TASK-DEVO-163 records the safe operating mode in `docs/architecture/real-codex-batch-run-readiness-checkpoint.md`: narrow approved policies, one item at a time, strict JSON output, manual review, manual validation, and trusted-runner-only delivery. TASK-DEVO-164 proves the same pattern on the live DevOrchestrator repo with two docs-only queue items, TASK-DEVO-165 adds the consolidated summary command to reduce artifact-joining overhead, and TASK-DEVO-166 dogfoods that summary against completed live policy `POL-0002`. TASK-DEVO-167 shows a new blocked-but-safe code-task outcome: a real Codex subprocess could inspect approved files but could not update existing source/test files. When worker evidence is `blocked` for write-access reasons, do not record review, validation, or delivery; diagnose the launcher/process write context or use patch-proposal fallback. TASK-DEVO-169 makes that fallback visible: a blocked/failed worker result can carry `patch_proposal_present` and a patch artifact path, but Devo keeps the result non-successful and tells the operator to review the patch manually before any normal gates. TASK-DEVO-170 dogfoods that path and confirms `queue-worker-evidence` plus `codex-worker-batch-summary` show the patch proposal path without recommending review, validation, delivery, commit, or push for patch-only evidence. Real Codex can spend usage, so use it only when the policy is tight enough to justify the run. Larger dogfood should continue one disposable queue item at a time before any future parallelism.

Patch proposal application is intentionally separate from patch proposal capture. TASK-DEVO-171 designs reviewed patch apply in `docs/architecture/reviewed-patch-apply-design.md`, and TASK-DEVO-172 adds the first safe inspection slice: `patch-proposal-show` plus explicit `patch-proposal-check --confirm-check`. The check command writes a workspace-only artifact and may run `git apply --check`, but it does not apply the patch, mutate queue state, record review, record validation, create delivery, commit, or push. TASK-DEVO-173 dogfoods those commands against existing fake blocked evidence and confirms the summary path should direct operators to `patch-proposal-show`/`patch-proposal-check`, not normal review/validation/delivery. TASK-DEVO-174 adds explicit reviewed apply. Apply requires a prior successful matching check artifact, re-verifies the patch hash and policy scope, writes an apply artifact, and leaves files unstaged. Patch apply still does not mean the queue item is complete: the operator must review the actual diff, run validation, record normal evidence only after the applied change is reviewed and validated, and deliver through the trusted runner.

TASK-DEVO-175 dogfoods the apply path with a fake safe docs patch in `docs/dogfood/task-devo-175-patch-proposal-apply-dogfood.md`. That report shows the intended sequence: blocked worker evidence, show, check, reviewed apply, unchanged queue/review/validation/delivery gates, manual diff inspection, validation, then normal evidence and trusted delivery.

TASK-DEVO-176 adds the bridge needed by the first real patch-proposal dogfood: if a confirmed `codex-worker-ingest` sees inline patch proposal text but no patch artifact path, it materializes the patch under `workspace/projects/<project>/planning/patch-proposals/artifacts/<QWR-ID>/<CWI-ID>.patch` and stores that path for `patch-proposal-show`, `patch-proposal-check`, and later reviewed apply. TASK-DEVO-177 makes the worker-result contract explicit by adding canonical `patch_proposal_text`; if the worker cannot create a patch artifact but knows the safe change, it should put a unified diff in that field and keep status blocked/failed. TASK-DEVO-178 proves that inline patch text is preserved, then tightens the contract because the first real patch was semantically useful but not `git apply` compatible. Inline patch text must now be a complete git-apply-compatible unified diff with `diff --git` headers, repo-relative file headers, and correct hunk counts. Corrupt/snippet patches stay blocked; request a fresh worker result rather than forcing apply or hand-editing outside the reviewed patch flow. This is still blocked evidence, not applied work, and dry-run ingest remains non-mutating.

TASK-DEVO-179 adds explicit whitespace-tolerant patch check/apply mode for the case where strict `git apply --check` fails but manual diagnostics prove the patch applies with Git whitespace tolerance. Strict mode remains the default. Use both `--ignore-whitespace` and `--confirm-ignore-whitespace` with `patch-proposal-check` to run `git apply --check --ignore-space-change --ignore-whitespace`; use the same flags with `patch-proposal-apply` only after human review. Apply requires a latest successful check for the same queue-worker run, same patch hash, and same patch apply mode. Whitespace-tolerant mode is audited in artifacts and still does not stage, commit, push, record normal evidence, create delivery, or complete the queue item.
...
Subprocess stdin, stdout, and stderr use explicit UTF-8 text handling. Invalid output bytes are replaced in the saved logs instead of crashing the subprocess reader, so useful surrounding output and worker diagnostics remain available on Windows regardless of the active console code page.
