...
TASK-DEVO-176 adds the bridge needed by the first real patch-proposal dogfood: if a confirmed `codex-worker-ingest` sees inline patch proposal text but no patch artifact path, it materializes the patch under `workspace/projects/<project>/planning/patch-proposals/artifacts/<QWR-ID>/<CWI-ID>.patch` and stores that path for `patch-proposal-show`, `patch-proposal-check`, and later reviewed apply. This is still blocked evidence, not applied work, and dry-run ingest remains non-mutating.
