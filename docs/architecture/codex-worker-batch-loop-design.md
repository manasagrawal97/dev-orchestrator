...
- worker evidence is patch-only; use `patch-proposal-show` and `patch-proposal-check --confirm-check` for inspection, then review/apply through a separate explicit future flow before normal review/validation/delivery
...
The reviewed apply path is designed separately in `docs/architecture/reviewed-patch-apply-design.md`. Future implementation should start with read-only show/check commands before any apply command exists; even after apply, normal review, validation, delivery, trusted runner, and queue completion gates still remain.
