...
`devo delivery latest --project <project>` is the read-only status shortcut for deciding what to do next. It reads existing checks, plans, approvals, reports, commit results, push results, and current Git status. It does not write artifacts or mutate the target repository. Empty clean checks are treated as informational, while older pushed deliveries remain visible as historical success.

`devo delivery runner-request` and `devo delivery runner-run` add a trusted local runner bridge for contexts where Codex/sandbox can edit and validate but cannot create `.git/index.lock`. A runner request is a workspace-only artifact that captures the expected changed-file snapshot and prints one normal PowerShell command. The runner command is explicit, foreground, and confirmation-gated; it is not a background daemon, webhook, auto-commit service, or UI action. Runner execution still performs readiness check, forbidden-path and secret-risk gates, delivery approval artifact creation/approval, delivery report preparation, commit-preview logic, guarded commit with index-lock preflight, push-preview logic, and guarded push. It blocks if the current changed files differ from the request snapshot or if any existing safety gate blocks.
