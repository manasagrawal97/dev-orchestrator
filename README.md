...
`runner-schedule-status` and `runner-schedule-doctor` report scheduler health as `healthy`, `disabled`, `not_installed`, `drift`, or `unknown`. `drift` means Devo metadata says the runner is enabled but the Windows scheduled task is missing. Auto-delivery should not be trusted until drift is repaired with `runner-schedule-install`, `runner-schedule-enable`, and another status check. Direct `runner-run --request <REQ-ID>` remains the safe fallback.
