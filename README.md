...
The long-term product vision is documented in [Company-model vision](docs/devo-company-model.md): Devo should behave like a local software-development company operating system around AI workers, with Codex CLI as the default personal/local worker and optional API/model agents deferred. Phase 1 MVP is complete and recorded in [Phase 1 MVP checkpoint](docs/phase-1-mvp-checkpoint.md). Future work should follow the post-Phase-1 priorities in [Remaining roadmap](docs/remaining-roadmap.md).
...
`devo delivery runner-request --project <project> --message "<commit message>" --note "<note>"` lets Codex or another restricted context prepare a trusted local delivery request under `workspace/projects/<project>/delivery/runner-requests/` without staging, committing, or pushing. It captures the current changed-file snapshot, refuses clean repos unless `--allow-empty-request` is explicit, refuses forbidden paths, staged workspace artifacts, and secret-risk blockers, and prints changed/warning/blocker counts plus one normal PowerShell command for the operator. `devo delivery runner-latest --project <project>` shows the latest request, latest run, expected changed-file count, commit/push status, and the exact runner command when a request is pending; use `runner-show` when blockers or an older request need detail. `devo delivery runner-run --project <project> --request <requestId> --approver "Manas" --confirm-runner-delivery` is intended for normal local PowerShell with `.\.venv\Scripts\devo.exe`; it reuses the existing delivery check, plan, approval, report, commit-preview, guarded commit, push-preview, and guarded push gates. It is explicit runner execution, not a daemon, webhook, UI button, auto-commit, or safety bypass.
