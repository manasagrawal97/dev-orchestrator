...
- [Reviewed patch-apply design](docs/architecture/reviewed-patch-apply-design.md)
...
- [Phase 1 MVP checkpoint](docs/phase-1-mvp-checkpoint.md)
- [Phase 2 autonomy roadmap](docs/phase-2-autonomy-roadmap.md)
- [Codex launcher setup runbook](docs/runbooks/codex-launcher-setup.md)
- [Codex subprocess write-access diagnostics](docs/runbooks/codex-subprocess-write-access-diagnostics.md)
...
- [TASK-DEVO-158 real Codex batch-run one-item dogfood](docs/dogfood/task-devo-158-real-codex-batch-run-one-item.md)
...
The intake commands are workspace/read-only helpers around the existing Project Brief -> Blueprint -> Backlog -> Batch -> Queue -> Codex handoff path. `intake-status` summarizes planning artifacts, task/batch/queue/handoff counts, progress percentages, and the next command. `intake-next` prints only the next action. `intake-template --write` and `intake-prompt --write` create `intake-template.md` and `intake-prompt.md` under `workspace/projects/<project>/planning/`. `intake-plan` parses a rough Markdown goal file into a deterministic planning bundle with candidate tasks, draft batch/queue/policy suggestions, allowed files, risk notes, and next commands; it previews by default and writes `workspace/projects/<project>/planning/intakes/<INTAKE-ID>/` only with `--confirm-create`. After review, `intake-materialize` converts that intake into draft backlog tasks, a draft batch, a draft queue, and a draft execution policy while preserving allowed files, do-not-touch notes, validation notes, delivery notes, and risk notes. `intake-next-slice` is read-only and recommends the smallest safe next slice from those materialized artifacts, including a warning when the broad materialized policy should stay draft and an exact narrow-policy command shape with preserved `--validation-command` flags. It deprioritizes already-performed setup tasks, such as running/reviewing the intake, when the intake and materialization artifacts already exist. `intake-policy-create-next` uses that recommendation to create one narrow draft execution policy for the next task/queue item while preserving the recommended allowed task, queue item, allowed files, forbidden files, validation commands, and notes. Preview mode is read-only; confirmed mode creates only a draft policy. These commands do not call AI, approve work, run Codex, run validation, create delivery requests, commit, push, or modify the target project.
...
Queue-worker run artifacts are stored under `workspace/projects/<project>/planning/queue-worker-runs/`. The assisted one-step path is `queue-worker-step`: it creates one queue-worker run if no active run exists for the approved policy, advances only one evidence gate at a time, creates a trusted delivery runner request when the run is ready, and later observes trusted runner completion without running runner-watch itself. `queue-worker-loop` repeatedly invokes that one-step behavior until the next safe stop condition, such as missing worker report, missing review, missing validation, pending trusted delivery, paused/failed/blocked state, no eligible item, or max steps. `approved-queue-run` is the v1 operator-friendly wrapper for that loop: it previews policy readiness first, requires `--confirm-auto-run` for mutation, can dry-run read-only, checks trusted runner scheduler health by default, and stops at the same evidence/delivery/failure boundaries. `--continue-next` can start one next eligible item after a specified run completes safely, and now reports a terminal no-action state when that item finishes the allowed policy scope. `codex-worker-batch-summary` is read-only and joins policy, queue item, queue-worker run, Codex preparation/batch/ingest, review, validation, delivery request, runner run, commit, push, and the single safe next command. If a restricted Codex/sandbox context reports scheduler drift while normal PowerShell reports healthy, use the normal-PowerShell evidence and explicit `--no-require-scheduler-healthy` only when the operator accepts that fallback. The record commands provide the manual evidence intake bridge: after a human/Codex worker finishes, record worker result, review, and validation evidence, then run `queue-worker-loop` or `approved-queue-run` again to advance through the gates. It can complete a queue item only after trusted delivery is completed and the existing queue completion checks pass, then it stops again at the next `waiting_worker` boundary. The lower-level commands remain available for explicit inspection and recovery. These commands do not run real Codex, call AI APIs, run validation, execute review, execute runner-watch, stage, commit, push, or modify target project source directly.
...
TASK-DEVO-171 adds the reviewed patch-apply design in [Reviewed patch-apply design](docs/architecture/reviewed-patch-apply-design.md). TASK-DEVO-172 implements the first safe inspection layer: `devo project patch-proposal-show` and `devo project patch-proposal-check --confirm-check`. The check path writes workspace artifacts and may run `git apply --check`, but it does not apply patches, mutate queue state, record review or validation, create delivery, commit, or push.

TASK-DEVO-173 dogfoods show/check against the existing fake blocked `QWR-0005` evidence. The commands stay non-mutating, the older proposal is safely rejected as non-applyable by `git apply --check`, and batch summary now points operators to `patch-proposal-show` for patch-only evidence.
...
TASK-DEVO-176 through TASK-DEVO-179 harden the real Codex write-blocked fallback. Confirmed ingest can materialize inline `patch_proposal_text` into a workspace `.patch` artifact, but that text must be a complete `git apply`-compatible unified diff. If strict `patch-proposal-check` fails, do not apply or force-edit around it. Corrupt patches need a fresh worker result; whitespace-only drift can use explicit audited `--ignore-whitespace --confirm-ignore-whitespace` check/apply mode, with apply still requiring a same-run, same-hash, same-mode successful check artifact.
...
devo delivery report-prepare --project MyProject --plan DEL-0001
devo delivery report-list --project MyProject
devo delivery report-show --project MyProject --report DEL-0001
devo delivery report-refresh --project MyProject --report DEL-0001 --note "permission issue cleared"
devo delivery report-refresh --project MyProject --report DEL-0001 --reopen --note "permission issue cleared"
devo delivery commit-message --project MyProject --plan DEL-0001
devo delivery commit-diagnostics --project MyProject --report DEL-0001
devo delivery commit-diagnostics --project MyProject --report DEL-0001 --index-lock-probe --confirm-probe
devo delivery commit-preview --project MyProject --report DEL-0001
devo delivery commit --project MyProject --report DEL-0001 --confirm-commit
devo delivery commit-show --project MyProject --delivery DEL-0001
devo delivery push-preview --project MyProject --report DEL-0001
devo delivery push --project MyProject --report DEL-0001 --confirm-push
devo delivery push-show --project MyProject --delivery DEL-0001
devo delivery runner-request --project MyProject --message "feat: deliver safely" --note "Validated by Codex"
devo delivery runner-latest --project MyProject
devo delivery runner-list --project MyProject
devo delivery runner-show --project MyProject --request REQ-0001
devo delivery runner-watch --project MyProject --approver "Manas" --once --confirm-runner-watch
devo delivery runner-run --project MyProject --request REQ-0001 --approver "Manas" --confirm-runner-delivery
devo delivery runner-schedule-plan --project MyProject --approver "Manas" --interval-minutes 5
devo delivery runner-schedule-install --project MyProject --approver "Manas" --interval-minutes 5 --dry-run --confirm-install
devo delivery runner-schedule-status --project MyProject
devo delivery runner-schedule-enable --project MyProject --confirm-enable
devo delivery runner-schedule-disable --project MyProject --confirm-disable
devo delivery runner-schedule-run-now --project MyProject --confirm-run-now
devo delivery runner-schedule-remove --project MyProject --confirm-remove
```

`devo delivery check` is read-only. It summarizes target repository branch/remote/status, changed/staged/unstaged/untracked files, forbidden path changes, workspace artifacts staged for commit, secret-risk files/signals, linked queue item status, linked worker review status, linked validation evidence, blockers, warnings, and the next safe action. With `--write`, it stores JSON/Markdown under `workspace/projects/<project>/delivery/` plus `delivery-index.json`. It does not stage, unstage, validate, commit, push, complete queue items, run Codex, run target commands, or modify target repositories. A known unreadable global Git ignore warning is reported as non-blocking noise when Git status/diff otherwise pass.

`devo delivery plan` creates a delivery plan from a written readiness check and records the intended future commit message, copied readiness evidence, changed-file summary, blockers, warnings, and approval status. `devo delivery approval-request` records that a plan needs delivery review. `devo delivery approve` approves only non-blocked delivery plans; warnings may be approved but remain recorded. `devo delivery reject` preserves the plan and records rejection. Delivery approval is not automatic commit or automatic push approval; each execution step still requires its own explicit confirmation flag.

`devo delivery report-prepare` creates a draft delivery report only after a delivery plan has been approved. It re-checks current readiness, records changed/staged/unstaged/untracked files, validation/review/safety summaries, blocker and warning summaries, approval status, commit readiness, push readiness, and the proposed commit message. The readiness fields are a report-preparation snapshot; after commit or push they are labeled historical, and operators should run a new delivery check if current repository state matters. `devo delivery commit-message` prints only the proposed commit message so the operator can copy it or compare it with a future override. A blocked current readiness check creates a blocked report instead of pretending delivery is ready.

`devo delivery latest --project <project>` is a read-only operator summary for the latest useful delivery state. It shows the current Git status, latest delivery check, latest meaningful non-empty check, latest plan/approval/report, latest commit and push results, latest pushed delivery, latest runner request/run/commit/push state, and the next recommended command. Empty clean delivery checks are informational: if there are no changed, staged, unstaged, or untracked files, `delivery latest` says no delivery is needed instead of nudging the operator to create a plan.

`devo delivery runner-request --project <project> --message "<commit message>" --note "<note>"` lets Codex or another restricted context prepare a trusted local delivery request under `workspace/projects/<project>/delivery/runner-requests/` without staging, committing, or pushing. It captures the current changed-file snapshot, refuses clean repos unless `--allow-empty-request` is explicit, refuses forbidden paths, staged workspace artifacts, and secret-risk blockers, and prints changed/warning/blocker counts plus one normal PowerShell command for the operator. `devo delivery runner-watch --project <project> --approver "Manas" --once --confirm-runner-watch` is now the preferred normal-PowerShell delivery command because it finds the oldest pending request automatically and then reuses the same `runner-run` delivery logic. `runner-run --request <requestId>` remains the precise fallback for a known request. `devo delivery runner-latest --project <project>` and `runner-watch-latest` show recent request/watch state. These commands reuse delivery check, plan, approval, report, commit-preview, guarded commit, push-preview, and guarded push gates. Watch mode is not a daemon, webhook, UI button, auto-commit, or safety bypass.

When running pytest validation, prefer an explicit root basetemp such as `--basetemp=E:\DevOrchestrator\pt-<task>-full`. Root `pt-*` folders are generated temp output and are ignored by Git, but they should not be committed. Before trusted delivery, remove leftover temp folders when possible:

```powershell
Remove-Item -Recurse -Force .\pt-* -ErrorAction SilentlyContinue
```

`devo delivery runner-schedule-plan` and `runner-schedule-status` make the background trusted runner understandable before it is enabled. `runner-schedule-install --dry-run --confirm-install` writes the planned config/status/wrapper artifacts without touching Windows Task Scheduler. A real install is explicit, disabled by default, and creates a local Windows scheduled task that runs `runner-watch --once` from the normal user context at the requested interval. Enabling, disabling, run-now, and removal each require their own confirmation flag. The scheduled runner processes at most one pending delivery request per trigger and still preserves every delivery safety gate; it is a local operational convenience, not an AI agent or a UI delivery control.

`runner-schedule-status` and `runner-schedule-doctor` report scheduler health as `healthy`, `disabled`, `not_installed`, `drift`, or `unknown`. `drift` means Devo metadata says the runner is enabled but the current process cannot confirm the Windows scheduled task. The task may be missing, or the current process may not be able to see Windows scheduled tasks. Verify from normal PowerShell before reinstalling repeatedly. If normal PowerShell is healthy but Codex/sandbox still reports drift, treat it as an environment visibility mismatch. Direct `runner-run --request <REQ-ID>` remains the safe fallback.

`devo delivery report-refresh` is the supported recovery path after a transient guarded commit failure. Without `--reopen`, it is refresh-only: it re-runs the current readiness snapshot, preserves previous commit failure metadata/history, and prints whether reopening would be allowed. With `--reopen`, it can move a blocked report back to `ready` only when the previous failure is classified retryable, the linked delivery plan and approval are still approved, the report has not already committed or pushed, and current readiness has no blockers. It does not stage, unstage, commit, push, validate, run Codex, or modify the target repo. After reopening, run `devo delivery commit-preview` again before any guarded commit.

`devo delivery commit-diagnostics` is read-only by default. It shows the target repo, branch/upstream, Git executable/version, `.git` path, `.git/index` status, `.git/index.lock` presence, current staged/unstaged/untracked files, delivery report status, last commit failure category/message/retryability, likely causes, and safe next actions. Retryable index-lock failures can be caused by Git/index conflicts, ACL or OS permission problems, antivirus/Controlled Folder Access, terminal/user permission mismatch, or read-only/protected `.git` state. The optional `--index-lock-probe --confirm-probe` is double-gated and attempts to create/remove `.git/index.lock` only for explicit diagnostics. Guarded commit also runs its own automatic index-lock preflight before staging; if that preflight fails, no files should be staged and the failure is recorded as retryable guidance. A retryable failure does not mean it is safe to retry immediately.
...
Project overview read models now include the latest delivery readiness id/status, blocker count, warning count, latest delivery plan id/status, latest approval status, latest delivery report id/status, commit-ready and push-ready flags, latest commit hash/status/pushed fields, latest push status/remote/branch/timestamp fields, and next action. The dashboard has a read-only Delivery page that shows those artifacts and copyable CLI commands only; commit and push remain CLI-only with explicit confirmation flags.
